...
*/
	...
	@Autowired
	private AuthenticationConfiguration authenticationConfiguration; //para jwt
 ...
 */
	...
	// Autorizacion a Rutas  ACL 
		...
		public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
			http.authorizeHttpRequests(auth -> auth
					//Rutas Publicas
					.requestMatchers("/", "/css/**", "/js/**", "/images/**", "/listar**", "/locale").permitAll() // , "/api/clientes/**"  listar**  al agregar ** permite aceptar todos los caracteres que vengan despues de la palabra listar
	...
	/*/
			/*//Otra forma de login y logout
			http.formLogin(login -> login.loginPage("/login").successHandler(successHandler).permitAll())
			http.logout(logout -> logout.logoutSuccessUrl("/login?logout").permitAll().addLogoutHandler(new HeaderWriterLogoutHandler(new ClearSiteDataHeaderWriter(Directive.COOKIES))))
			 */
	//		http.exceptionHandling(((exceptionHandling) -> exceptionHandling.accessDeniedPage("/error_403")));
...
*/			
			...
			//Se deshabilita csrf porque se usará el token de jwt y no el token de proteccion csrf
/*			try {
	...
	*/		
			...
			http.httpBasic(Customizer.withDefaults()); //puede ser eliminado
			return http.build();
		}
...
/*//Otra forma
	...
	public SecurityFilterChain filterChain(HttpSecurity http)throws Exception {
		...
		.exceptionHandling(((exceptionHandling) -> exceptionHandling.accessDeniedPage("/error_403"))).csrf(csrf -> csrf.disable())
